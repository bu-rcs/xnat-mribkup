...
logfile="runlog-v2-240603GG.MR.INVESTIGATORS_S.6-${rundate}.log"
...
find /cnc/DATA/tmp -type f -name "*240603GG.MR.INVESTIGATORS_S.6*.dcm" -print0 | tac -s '' | sort -z | while IFS= read -r -d '' path; do
    echo "Starting ..."
    echo "$path"
    ...
    if [[ $InvestigatorfromHeader =~ "ExternalUsers" ]]; then
            newpath_prefix="/cnc/DATA/INVESTIGATORS/EXTERNALUSERS_"
    elif [[ $InvestigatorfromHeader =~ "INVESTIGATORS" ]]; then
    ...
    newpath="${newpath_prefix}${Investigator}/${IDfromHeader}"
